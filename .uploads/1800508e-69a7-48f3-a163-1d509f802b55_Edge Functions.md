...
Edge Functions 运行在 EdgeOne 全球边缘节点上，提供 Serverless 代码执行环境。您只需编写业务函数代码并部署到 Makers 项目中，即可在靠近用户的边缘节点上弹性、安全地运行代码，无需配置和管理服务器等基础设施。
...
您无需再关心和维护底层服务器的内存、CPU、网络和其他基础设施资源，可以抽出精力更专注于业务代码的开发。

### 快速开始

在项目的 ./edge-functions/api 目录下新建 hello.js，使用以下示例代码创建您的第一个 Edge Function：
``` typescript
...
上述目录文件结构，经 EdgeOne Makers 平台构建后将生成以下路由。这些路由将 Makers URL 映射到 `/edge-functions` 文件，当客户端访问 URL 时将触发对应的文件代码运行：
<table>
<tr>
...
使用 Function Handlers 可为 Makers 创建自定义请求处理程序，以及定义 RESTful API 实现全栈应用。支持下述 Handlers 方法：
<table>
<tr>
